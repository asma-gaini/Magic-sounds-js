...
const page_lenght = data_music_favorit.length / page_size;
...
var favoritHeart = "../image/svg/heart-full.svg";
...
function createPlayerTemplate() {
  ...
  template.innerHTML = createFavoritPlayer();
}

function createFavoritPlayer() {
  let getPlayerId = document.querySelector("#favoritPlayList");
...
function createItemOfPlaylist(ulTag) {
  for (let i = start_indexSong; i < end_indexSong; i++) {
    ...
    imageDiv.setAttribute("id", data_music_favorit[i].id);
    ...
    imageTag.setAttribute("src", data_music_favorit[i].image);
    imageTag.setAttribute("alt", data_music_favorit[i].title);
    ...
    titleLink.innerHTML = data_music_favorit[i].title;
    ...
    subtitleLink.innerHTML = data_music_favorit[i].artist;
    ...
    songTime.innerHTML = data_music_favorit[i].time;
    ...
    favoritSvg.setAttribute("id", data_music_favorit[i].id);
...
function createModal() {
  let getModalId = document.querySelector("#favoritPlayList");
...
function handlePlayPause() {
  ...
  for (let i = 0; i < data_music_favorit.length; i++) {
    if (data_music_favorit[i].audioSrc == myAudioSrc) {
      const id = data_music_favorit[i].id;
...
function setSongInformation(clickedId) {
  ...
  for (let i = 0; i < data_music_favorit.length; i++) {
    if (data_music_favorit[i].id == clickedId) {
      document
        .getElementById("single-song")
        .setAttribute("src", data_music_favorit[i].audioSrc);
      document
        .getElementById("single-song-image")
        .setAttribute("src", data_music_favorit[i].image);
      document.getElementById("modalSong-title").innerHTML =
        data_music_favorit[i].title;
      document.getElementById("modalSong-artistName").innerHTML =
        data_music_favorit[i].artist;
      setText(clickedId);
      return;
    }
  }
...
function setText(clickedId) {
  ...
  let textPoemArray = data_music_favorit[clickedId - 1].textMusic.split("/");
...
function autoPlayNextSong() {
  ...
  aud.onended = function () {
    ...
    for (let i = 0; i < data_music_favorit.length; i++) {
      if (data_music_favorit[i].audioSrc == audioSrc) {
        let currntSongId = parseInt(data_music_favorit[i].id);
